...
from lib.gvas import GvasHeader
...
with open('/Users/wangjingjin/Downloads/saves/Level.sav', 'rb') as f:
    data = f.read()
...
host_sav_path = '/Users/wangjingjin/Downloads/saves/Players/F38FA3AE000000000000000000000000.sav'
host_sav_basename = os.path.basename(host_sav_path).split('.')[0]
with open(host_sav_path, 'rb') as f:
    data = f.read()
    raw_gvas, _ = decompress_sav_to_gvas(data)
...
host_guid = UUID.from_str(host_sav_basename)
print(host_guid)
...
print(host_instance_id)

# level_json["CharacterSaveParameterMap"]["value"]
...
targ_sav_path = '/Users/wangjingjin/Downloads/saves/Players/5AEAEC8B000000000000000000000000.sav'
targ_sav_basename = os.path.basename(targ_sav_path).split('.')[0]
with open(targ_sav_path, 'rb') as f:
    data = f.read()
    raw_gvas, _ = decompress_sav_to_gvas(data)

targ_json = GvasFile.read(raw_gvas).properties
targ_guid = UUID.from_str(targ_sav_basename)
print(targ_guid)

with open('/Users/wangjingjin/Downloads/Level.sav', 'rb') as f:
    data = f.read()
    raw_gvas, _ = decompress_sav_to_gvas(data)
...
targ_lvl, section_ranges = reader.load_sections([
    ('CharacterSaveParameterMap', MAP_START),
    ('ItemContainerSaveData', MAP_START),
    ('DynamicItemSaveData', ARRAY_START),
    ('CharacterContainerSaveData', MAP_START),
...
# # TODO: Check if these two lines are really needed
# host_json["SaveData"]["value"]["PlayerUId"]["value"] = targ_json["SaveData"]["value"]["PlayerUId"]["value"]
# host_json["SaveData"]["value"]["IndividualId"]["value"]["InstanceId"]["value"] = targ_json["SaveData"]["value"]["IndividualId"]["value"]["InstanceId"]["value"]
...
removed = 0
filter_ids = [inv_pals["value"]["ID"]["value"], inv_otomo["value"]["ID"]["value"]]
...
output_data = SkipFArchiveWriter(custom_properties=PALWORLD_CUSTOM_PROPERTIES).write_sections(targ_lvl, section_ranges, raw_gvas)
...
sav_file_data = compress_gvas_to_sav(
    output_data, save_type
)
with open('/Users/wangjingjin/Downloads/saves/NewLevel.sav', 'wb') as out:
