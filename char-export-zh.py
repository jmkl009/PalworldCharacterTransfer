...
def main():
    ...
    if None in [level_sav_path, t_level_sav_path, selected_source_player, selected_target_player]:
        messagebox.showerror(message='在开始迁移前请选择好所有Level文件和玩家')
        return
...
点击 Yes 如果你认为可以继续')
    ...
    if not found:
        ...
        messagebox.showerror(message="在来源世界里没有发现来源玩家的实际数据")
        return
    ...
    if count < expected_containers:
        ...
        messagebox.showerror(message="缺少容器信息! 只发现了 " + str(count))
        return
    ...
    if not found:
        ...
        messagebox.showerror(message="没有在目标世界中找到目标人物的实际数据")
        return
    ...
    if not keep_old_guild_id:
        ...
        if group_id is None:
            messagebox.showerror(message='没有发现公会信息')
            return
        ...
        if group_id is None: # No old guild containing the source player is found
            ...
            if old_guild is None:
                ...
                messagebox.showerror(message="来源人物未在来源世界公会中, 要么是一个bug要么是文件损坏")
                return
    ...
    messagebox.showinfo(message='迁移成功! 你可以选择更多人物进行迁移或者关闭此程序。')
...
def source_level_file():
    ...
    if tmp:
        ...
        if not raw_gvas:
            messagebox.showerror(message="文件不正确, 文件需要以 .sav结尾")
            return
        ...
        current_selection_label.config(text=f"来源: {selected_source_player}, 目标: {selected_target_player}")
...
source_player_list.heading(0, text='公会 ID')
source_player_list.heading(1, text='玩家 ID')
source_player_list.heading(2, text='玩家名字')
...
# Define the column headings
target_player_list.heading(0, text='公会 ID')
target_player_list.heading(1, text='玩家 ID')
target_player_list.heading(2, text='玩家名字')
...
current_selection_label = Label(root, text=f"来源: {selected_source_player}, 目标: {selected_target_player}",
                                wraplength=600)
...
checkbox_var = IntVar()
keep_old_guild_check = Checkbutton(root, text="在迁移后依旧保有旧公会ID", variable=checkbox_var, command=on_keep_old_guild_check)
keep_old_guild_check.grid(row=7, column=0, columnspan=2, sticky='w', padx=10, pady=5)

save_version_var = IntVar()
keep_old_guild_check = Checkbutton(root, text="输出旧版本存档", variable=save_version_var, command=on_output_old_save_version_check)
